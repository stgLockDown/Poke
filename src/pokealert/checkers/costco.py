"""Costco checker using the public product page."""
...
class CostcoChecker(BaseChecker):
    retailer = "costco"

    async def check(self, product: ProductConfig) -> StockResult:
        if not product.url:
            return self._error(product, "costco product requires a url")
        ...
        if state == StockState.UNKNOWN:
            ...
            if re.search(r"out of stock|sold out|no longer available", text):
                state = StockState.OUT_OF_STOCK
